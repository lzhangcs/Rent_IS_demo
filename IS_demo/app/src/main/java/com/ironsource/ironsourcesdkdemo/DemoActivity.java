...
public class DemoActivity extends Activity implements RewardedVideoListener, InterstitialListener, ImpressionDataListener {
    ...
    //linda
    private Button btn_noPtype;

    private Button btn_Ptype;
...
//linda good
        btn_noPtype = findViewById(R.id.btn_noPtype);
        btn_noPtype.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                //if read picture from sdcard, should requestPermission()
                switch (v.getId()) {
                    case R.id.btn_noPtype:
        ...
        btn_Ptype = findViewById(R.id.btn_Ptype);
        btn_Ptype.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                //if read picture from sdcard, should requestPermission()
                switch (v.getId()) {
    ...
    //picture size <=20MB, resolution <= 16384 X 8192
    ...
    private void PanoramaInterface_loadImageInfoWithType() {
        Uri uri = Uri.parse("android.resource://" + getPackageName() + "/" + R.raw.inside);
        Panorama.getInstance()
                .loadImageInfoWithPermission(this, uri, PanoramaInterface.IMAGE_TYPE_RING)
    ...
    private void PanoramaInterface_localInterface () {
        Uri uri = Uri.parse("android.resource://" + getPackageName() + "/" + R.raw.inside);
        Intent intent = new Intent(DemoActivity.this, LocalInterfaceActivity.class);
        intent.setData(uri);
        intent.putExtra("PanoramaType", PanoramaInterface.IMAGE_TYPE_RING);
